...
const DOMINI = "https://database-sc.alwaysdata.net/";
$(document).ready(function () {
  ...
  $("#search").keyup(function (e, table) {
    ...
    if ($("#search").val()) {
      ...
      if (search.includes("€")) {
        alert("Recorda buscar els preus sense el €");
      }
      if (search.includes("Pol")) {
        alert(":)");
      }
      $.ajax({
  ...
  function fetchTasks(limit, order, direction, table, table2) {
    ...
    url = DOMINI + "/API.php?viewDB";
    // console.log('1: ' + table + '. 2: ' + table2)
    $.post(
      url,
      { table, limit, order, direction, table2 },
      function (response) {
        if (response == "") {
  ...
  $(document).on("click", ".button_delete", function () {
    let advice =
      table === "backup"
        ...
        : "This action will delete this row forever (a lot of time)";
    if (confirm(advice)) {
